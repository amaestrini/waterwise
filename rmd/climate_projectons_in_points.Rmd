...
##%>% filter(model %in% n_models)
  ...
  #### for bokeh module
  outf$ttt <- as.data.frame(outf$zoov)
  outf$ttt$month <- month(as.Date(index(outf$zoov)))
  outf$ttt$year <- year(as.Date(index(outf$zoov)))
  outf$ttt <- melt(outf$ttt,id=c("year","month","observation")) %>% filter(!is.na(observation))
  names(outf$ttt)[names(outf$ttt)=="variable"] <- "model"
  
  
  ####

  outf$main2 <- sprintf("%s %s %s",varsel,station0,in_selected_global_model)
...
rrev(rr)
dygraph(rr$zoov,main=rr$main,xlab="Time",ylab=rr$ylab) %>% dyRangeSelector() %>% dyOptions(stepPlot=rr$stepPlot) %>% dySeries("observation", stepPlot = rr$stepPlot,color = "blue") 
})
# 
